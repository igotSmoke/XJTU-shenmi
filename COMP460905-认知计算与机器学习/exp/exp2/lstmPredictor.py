...
class LSTMForecaster:
    ...
    def prepare_data(self):
        data = pd.read_csv(self.data_path, header=None).values.astype(np.float32)
        ...
        def create_samples(dataset):
            X, y = [], []
            for i in range(len(dataset) - self.window_size - self.pred_len + 1):
                X.append(dataset[i:i+self.window_size])
                y.append(dataset[i+self.window_size:i+self.window_size+self.pred_len].flatten())
    ...
    def build_model(self):
        self.model = self.LSTMModel(1, self.hidden_size, self.num_layers, self.pred_len).to(self.device)
    ...
    def plot_results(self, actuals, predictions):
        ...
        plt.savefig('multi_step_prediction_Istm.png')
        plt.close()
