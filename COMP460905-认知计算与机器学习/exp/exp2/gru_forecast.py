...
class GRUForecaster:
    def __init__(self, data_path, window_size, pred_len, hidden_size, num_layers, epochs, batch_size, lr):
    ...
    def prepare_data(self):
        ...
        def create_samples(dataset):
            X, y = [], []
            for i in range(len(dataset) - self.window_size - self.pred_len):
                X.append(dataset[i:i+self.window_size])
                y.append(dataset[i+self.window_size:i+self.window_size+self.pred_len])
        ...
        self.y_train = torch.from_numpy(y_train).float().view(-1, self.pred_len)
        self.X_test = torch.from_numpy(X_test).float().view(-1, self.window_size, 1)
        self.y_test = torch.from_numpy(y_test).float().view(-1, self.pred_len)

    def build_model(self):
        self.model = self.GRUModel(1, self.hidden_size, self.num_layers, self.pred_len).to(self.device)
    ...
    def plot_results(self, actuals, predictions):
        ...
        for step in range(self.pred_len):
            plt.plot(actuals[:, step], label=f'Actual Step {step+1}', alpha=0.7)
            plt.plot(predictions[:, step], label=f'Predicted Step {step+1}', linestyle='--')
        plt.legend()
        plt.title(f'Multi-Step ({self.pred_len} Steps) Time Series Prediction')
        plt.savefig('multi_step_prediction_gru.png')
        plt.close()
